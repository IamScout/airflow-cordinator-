...
check_DONE = BashOperator(
	task_id='check.dir',
	bash_command='''
	if ls /etc/airflow/flag/teams-statistics-DONE; than rm /etc/airflow/flag/teams-statistics-DONE exit 0
	else exit 1 fi
	''',
	dag=dag
)
...
send_uri = BashOperator(
	task_id='send.uri',
	bash_command=f'''
	python3 /etc/airflow/src/uri/make_uri_teams_statistics.py
	''',
	dag=dag
)

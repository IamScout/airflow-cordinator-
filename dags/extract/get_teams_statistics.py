...
send_uri = BashOperator(
	task_id='send.uri',
	bash_command=f"""
	python3 {main_dir}/src/uri/make_uri_teams_statistics.py {date}
	""",
	dag=dag
)
...
make_DONE = BashOperator(
	task_id='make.DONE',
	bash_command=f"""
	curl 34.64.254.93:3000/check/teams-statistics/?cnt=895
	""",
	dag=dag
)
...
branch_check_DONE = BranchPythonOperator(
	...
	op_kwargs={"url":"34.64.254.93:3000/done-flag/?target_dir=/api/app/datas/json/season_22/teams-statistics/"},
	dag=dag
)
...
blob_job = BashOperator(
    task_id='blob.job',
    bash_command=f'''
	curl 34.64.254.93:3000/blob-data/?target_dir=/api/app/datas/json/season_22/teams-statistics
	''',
    dag=dag
)
...
clensing_data = BashOperator(
    ...
    bash_command='''
	curl 34.64.254.93:3000/delete/teams-statistics/
	''',
    dag=dag
)
...
send_noti = BashOperator(
    ...
    curl -X POST -H 'Authorization: Bearer fxANtArqOzDWxjissz34JryOGhwONGhC1uMN8qc59Z3'
                 -F 'Something is wrong with today's teams/statistics data' https://notify-api.line.me/api/notify
    ''',
    dag=dag
)

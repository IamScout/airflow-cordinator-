...
check_DONE = BashOperator(
	task_id='check.dir',
	bash_command="""
	if ls /opt/airflow/flag/fixtures-lineups-DONE; then rm /opt/airflow/flag/fixtures-lineups-DONE exit 0
	else exit 1 fi
	""",
	dag=dag
)
...
send_uri = BashOperator(
	task_id='send.uri',
	bash_command="""
	python3 /opt/airflow/src/uri/make_uri_fixtures_lineups.py '{{execution_date.strftime('%Y-%m-%d')}}'
	""",
	dag=dag
)
...
make_DONE = BashOperator(
	task_id='make.DONE',
	bash_command="""
	touch /opt/airflow/flag/fixtures-lineups-DONE
	""",
	dag=dag
)
...
make_ERROR = BashOperator(
	task_id='make.ERROR',
	bash_command="""
	touch /opt/airflow/flag/fixtures-lineups-ERROR
	""",
	dag=dag,

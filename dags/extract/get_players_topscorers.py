...
dag = DAG('load_players_topscorers_data',
		  ...
		  tags=['Extract','players-topscorers'],
		  max_active_runs=1,
...
check_DONE = BashOperator(
	task_id='check.dir',
	bash_command='''
	if ls /etc/airflow/flag/players-topscorers-DONE; than rm /etc/airflow/flag/players-topscorers-DONE exit 0
	else exit 1 fi
	''',
	dag=dag
)
...
send_uri = BashOperator(
	task_id='send.uri',
	bash_command=f'''
	python3 /etc/airflow/src/uri/make_uri_players_topscorers.py
	''',
	dag=dag
)

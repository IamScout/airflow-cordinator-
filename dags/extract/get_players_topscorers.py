...
# DAG SETTINGS
dag = DAG('load_players_topscorers_data',
		  default_args=default_args,
		  tags=['Extract','players_topscorers'],
		  max_active_runs=1,
...
send_uri = BashOperator(
	task_id='send.uri',
	bash_command=f"""
	python3 {main_dir}/src/uri/make_uri_players_topscorers.py
	""",
	dag=dag
)
...
make_DONE = BashOperator(
	task_id='make.DONE',
	bash_command=f"""
	curl 34.64.254.93:3000/check/players-topscorers/?cnt=55
	""",
	dag=dag
)
...
branch_check_DONE = BranchPythonOperator(
	...
	op_kwargs={"url":"34.64.254.93:3000/done-flag/?target_dir=/api/app/datas/json/season_22/players_topscorers/"},
	dag=dag
)
...
blob_job = BashOperator(
    task_id='blob.job',
    bash_command=f'''
	curl 34.64.254.93:3000/blob-data/?target_dir=/api/app/datas/json/season_22/players-topscorers
	''',
    dag=dag
)
...
clensing_data = BashOperator(
    ...
    bash_command='''
	curl 34.64.254.93:3000/delete/players-topscorers/
	''',
    dag=dag
)

...
send_uri = BashOperator(
	task_id='send.uri',
	bash_command="""
	python3 /opt/airflow/src/uri/make_uri_fixtures_players.py '{{execution_date.strftime('%Y-%m-%d')}}'
	""",
	dag=dag
)
...
make_DONE = BashOperator(
	task_id='make.DONE',
	bash_command="""
	touch /opt/airflow/flag/fixtures-players-DONE
	""",
	dag=dag
)
...
make_ERROR = BashOperator(
	task_id='make.ERROR',
	bash_command="""
	touch /opt/airflow/flag/fixtures-players-ERROR
	""",
	dag=dag,

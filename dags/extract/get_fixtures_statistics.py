...
check_DONE = BashOperator(
	task_id='check.dir',
	bash_command="""
	if ls /opt/airflow/flag/fixtures-statistics-DONE; then rm /opt/airflow/flag/fixtures-statistics-DONE exit 0
	else exit 1 fi
	""",
	dag=dag
)
...
send_uri = BashOperator(
	task_id='send.uri',
	bash_command="""
	python3 /opt/airflow/src/uri/make_uri_fixtures_statistics.py '{{execution_date.strftime('%Y-%m-%d')}}'
	""",
	dag=dag
)

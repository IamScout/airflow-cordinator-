...
# DAG SETTINGS
dag = DAG('load_players_squad_data',
...
send_uri = BashOperator(
	...
	bash_command=f"""
	python3 {main_dir}/src/uri/make_uri_players_squads.py
	""",
	dag=dag
)
...
def get_done_response(url):
    ...
    if output == "true":
        return "blob_players_squads_data_DL"
    else:
...
branch_check_DONE = BranchPythonOperator(
	...
	op_kwargs={"url":"34.64.254.93:3000/done-flag/?target_dir=/api/app/datas/json/season_22/players_squads/"},
	dag=dag
)
...
send_noti = BashOperator(
    ...
    curl -X POST -H 'Authorization: Bearer fxANtArqOzDWxjissz34JryOGhwONGhC1uMN8qc59Z3'
                 -F 'Something is wrong with today's players/squads data' https://notify-api.line.me/api/notify
    ''',
    dag=dag
)

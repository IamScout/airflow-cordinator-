...
check_DONE = BashOperator(
	task_id='check.dir',
	bash_command='''
	if ls /etc/airflow/flag/standings-DONE; than rm /etc/airflow/flag/standings-DONE exit 0
	else exit 1 fi
	''',
	dag=dag
)
...
send_uri = BashOperator(
	task_id='send.uri',
	bash_command=f'''
	python3 /etc/airflow/src/uri/make_uri_standings.py
	''',
	dag=dag
)
...
make_DONE = BashOperator(
	task_id='make.DONE',
	bash_command='''
	touch /opt/airflow/flag/standings-DONE
	''',
	dag=dag
)
...
make_ERROR = BashOperator(
	task_id='make.ERROR',
	bash_command='''
	touch /opt/airflow/flag/standings-ERROR
	''',
	dag=dag,

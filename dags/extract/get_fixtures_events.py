...
check_DONE = BashOperator(
	task_id='check.dir',
	bash_command='''
	if ls /etc/airflow/flag/fixtures-events-DONE; than rm /etc/airflow/flag/fixtures-events-DONE; exit 0
	else exit 1; fi
	''',
	dag=dag
)
...
send_uri = BashOperator(
	task_id='send.uri',
	bash_command=f'''
	python3 /etc/airflow/src/uri/make_uri_fixtures_events.py {execution_date}
	''',
	dag=dag
)

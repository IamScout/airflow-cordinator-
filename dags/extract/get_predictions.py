...
check_DONE = BashOperator(
	task_id='check.dir',
	bash_command="""
	if ls /opt/airflow/flag/predictions-DONE; then rm /opt/airflow/flag/predictions-DONE exit 0
	else exit 1 fi
	""",
	dag=dag
)
...
send_uri = BashOperator(
	task_id='send.uri',
	bash_command="""
	python3 /opt/airflow/src/uri/make_uri_predicitions.py '{{execution_date.strftime('%Y-%m-%d')}}'
	""",
	dag=dag
)
...
make_DONE = BashOperator(
	task_id='make.DONE',
	bash_command="""
	touch /opt/airflow/flag/predictions-DONE
	""",
	dag=dag
)
...
make_ERROR = BashOperator(
	task_id='make.ERROR',
	bash_command="""
	touch /opt/airflow/flag/predictions-ERROR
	""",
	dag=dag,
